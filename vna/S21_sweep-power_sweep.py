...
def save_data(folder, file, sample, myrun, freq_arr, s21_arr, power_arr, verbose=False):
    # Create folders if they do not exist
    if not os.path.isdir(folder):
        os.makedirs(folder)
    ...
    power_data_str = "{}/{}/bias_arr".format(sample, myrun)
    ...
    with h5py.File(os.path.join(folder, file), "a") as savefile:
        ...
        savefile.create_dataset(power_data_str, (np.shape(power_arr)), dtype=np.float64, data=power_arr)

        # Write dataset attributes
        savefile[freq_data_str].attrs["Unit"] = "Hz"
        savefile[power_data_str].attrs["Unit"] = "fsu"
...
# Power values
power_arr = np.linspace(-43, 0, 10)
s11_arr = np.zeros((len(power_arr), Npts), dtype=complex)

# loop over input power
with tqdm(total=len(power_arr), ncols=80) as pbar:
    for power_idx, power_val in enumerate(power_arr):
        # VNA frequency sweep
        data = sweep_vna(f_start, f_stop, intbw, f_delta, power_val, Navg, verbose)
        freq_arr = data['freq_arr']
        s11_arr[power_idx] = data['s21']

        pbar.update(1)

# Save data
save_data(save_folder, save_file, sample, myrun, freq_arr, s11_arr, power_arr, verbose)

...
with lockin.Lockin( address = ADDRESS,
    ...
    # Data
    usb_arr = np.zeros( (nr_df, nr_amp_pump, Npix, nr_freq), dtype=np.complex128)
    ...
    og.set_amplitudes( np.zeros_like(freq_arr)+0.001 )
    # Set the lock-in output phases
    og.set_phases( phases = np.zeros_like(freq_arr),
                   phases_q = np.zeros_like(freq_arr) )
    ...
    with tqdm( total=(nr_amp_pump * nr_df), ncols=80 ) as pbar:
        ...
        for df_ind, df_val in enumerate( df_arr ):
            ...
            for pamp_ind, pamp_val in enumerate(amp_pump_arr):
                ...
                # Store data in array
                usb_arr[df_ind, pamp_ind] = HSB[-Npix:]
                lsb_arr[df_ind, pamp_ind] = LSB[-Npix:]

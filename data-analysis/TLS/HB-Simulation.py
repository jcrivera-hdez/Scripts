...
def eom_driven(t, state, fr, κ0, λ, drive, ddrive):
    ...
    return np.sqrt(gain) * (-1.0j * ωr * (state_norm - drive_norm) - κ0 * (
            state_norm - drive_norm) + ddrive_norm + λ ** 2 * drive_norm)
...
def eom_nonlinear(t, state, fr, κ0, κ1, λ, drive, ddrive):
    ...
    drive_norm = drive(t) / np.sqrt(att)
    ddrive_norm = ddrive(t) / np.sqrt(att)
    return np.sqrt(gain) * (-1.0j * ωr * (state_norm - drive_norm) - κ0 * (state_norm - drive_norm) - κ1 * (
            state_norm - drive_norm) ** 3 + ddrive_norm + λ ** 2 * drive_norm)
...
def driven_simulation(f0, κ0, λ, f1, f2, F0):
    ...
    return A, Ain, a_all, a, ain, f, t, drive, max_ind[0]
...
def nonlinear_simulation(f0, κ0, κ1, λ, f1, f2, F0):
    ...
    return A, Ain, a_all, a, ain, f, t, drive, max_ind
...
# Simulation
A, Ain, a_all, a, ain, f, t, drive, max_ind = driven_simulation(f0, κ0, λ, f1, f2, F0)
...
fig0, ax = plt.subplots(3, 1, figsize=[9, 6])
...
# Simulation
A, Ain, a_all, a, ain, f, t, drive, max_ind = nonlinear_simulation(f0, κ0, κ1, λ, f1, f2, F0)
...
# "Power" values
F0_arr = np.linspace(0.01, 4.6, 11)
...
for F0_ind, F0_val in enumerate(F0_arr):
    # Simulation section
    A[F0_ind], Ain[F0_ind], a_all, a, ain, f, t, drive, max_ind = nonlinear_simulation(f0, κ0, κ1, λ, f1, f2,
                                                                                       F0_val)

    # Reconstruction section
    λ_recon[F0_ind], f0_recon[F0_ind], κ0_recon[F0_ind], κ1_recon[F0_ind], Q_fit, Q = recon_nonlinear(A[F0_ind],
                                                                                                      Ain[F0_ind], a,
                                                                                                      ain, f, max_ind)
...
ax[0].plot(F0_arr[1:], λ_recon[1:], '.-')
ax[1].plot(F0_arr[1:], f0_recon[1:], '.-')
ax[2].plot(F0_arr[1:], κ0_recon[1:], '.-')
ax[3].plot(F0_arr[1:], κ1_recon[1:], '.-')

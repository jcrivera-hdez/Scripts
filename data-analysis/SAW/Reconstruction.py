...
# fourpumps_ON_OFF_cycle_2khz_df_modejumping indices
f1_ind = 12
p1_ind = 6
...
sGsn = npzfile['sGsn']
freqs_calibration = npzfile['freq_comb'] # -14144897.4609375
...
cov_matrix = np.zeros(( part_len, 2*len(nmodes), 2*len(nmodes) ))
...
optimal_value = np.zeros(( part_len ))
...
kpart_val = np.zeros(( part_len, n_kpartitions ))
...
bipart_val = np.zeros(( part_len, n_bipartitions ))
...
for part_ind in range(part_len):
    ...
    cov_matrix[part_ind] = np.cov( coord_array ) * 6.33645961**2
    ...
    cov_matrix_std[part_ind] = np.std( part_of_cov[:, :, :], axis = 0, ddof = 1 ) / np.sqrt( n_parts ) * 6.33645961**2
    ...
    for i in range( len(cov_matrix[part_ind]) ):
        for j in range( len(cov_matrix[part_ind]) ):
                cov_matrix_exp[i,j] = cov_matrix[part_ind, i, j] / conv_factor / np.sqrt(freqs_comb[i//2]) / np.sqrt(freqs_comb[j//2])
                cov_matrix_exp_std[i,j] = cov_matrix_std[part_ind, i, j] / conv_factor / np.sqrt(freqs_comb[i//2]) / np.sqrt(freqs_comb[j//2])
    ...
    # RECONSTRUCTION OF THE COVARIANCE MATRIX
    cov_matrix_reconstructed[part_ind] = T_inv.dot( cov_matrix_exp - 
    ...
    cov_matrix_reconstructed_std[part_ind] = np.sqrt ( sA1 + sA2 + sB + sC1 + sC2 + corr_gsgi + corr_gsn + corr_gin )
    ...
    evaluation_block = cp.abs( (cov_matrix_reconstructed[part_ind] - quantum_cov) / cov_matrix_reconstructed_std[part_ind] )
    ...
    if prob_alt.value is None:
        ...
        optimal_value[part_ind] = 100
        
        print("\nThe optimal value is", optimal_value[part_ind])
        
        ent_test[part_ind] = 1
        ent_error[part_ind] = 1
    ...
    # Reconstruction value (normalised to std)
    optimal_value[part_ind] = prob_alt.value
    
    # Print result
    print("\nThe optimal value is", optimal_value[part_ind])
    
    # QUANTUM RECONSTRUCTED COVARIANCE MATRIX
    cov_matrix_quantum[part_ind] = quantum_cov.value
    ...
    #         ppt_cov_on = ppt_matrix.dot( cov_matrix_quantum[part_ind] ).dot( ppt_matrix )
    #         heisenberg_matrix_on = ppt_cov_on + 1j*symp
    #         heisenberg_eigvals_on = np.linalg.eigvals( heisenberg_matrix_on )
            
    #         min_eig_ppt[part_ind, ii-1] = np.real( np.min(heisenberg_eigvals_on) )
            
    #         if min_eig_ppt[part_ind, ii-1] > 1e2:
    #             min_eig_ppt[part_ind, ii-1] = 1
    ...
    x0 = np.random.rand( len(cov_matrix_quantum[part_ind]) // 2) * 2 * np.pi 
        
    res = minimize(lambda phi: decouple_IQ( cov_matrix_quantum[part_ind], phi), x0, method='nelder-mead', 
                   options={'maxiter': 80000, 'xtol': 1e-10, 'disp': False, 'ftol': 1e-10})
    x0 = res.x
    
    bounds = [(-10, 10) for i in range( len(cov_matrix_quantum[part_ind]) ) ]
    
    # Rotation of the data
    cov_matrix_quantum_rot[part_ind] = rotation_cov_matrix( cov_matrix_quantum[part_ind], x0 )
    cov_matrix_quantum_rot_err[part_ind] = rotation_cov_matrix( cov_matrix_reconstructed_std[part_ind], x0 )
    
    reconstructed_on_parsed = cov_matrix_quantum_rot[part_ind]
    reconstructed_on_parsed_err = cov_matrix_quantum_rot_err[part_ind]
    
    n_tries = 3
    ent_ratios = np.zeros( n_tries )
    ent_vectors = np.zeros(( n_tries, len( cov_matrix_quantum[part_ind] ) ))
    ...
    ent_test[part_ind], ent_error[part_ind] = entanglement_criteriaN_( ent_vectors[ent_ratios == np.min(ent_ratios)][0], reconstructed_on_parsed, reconstructed_on_parsed_err)
    ...
    #     reconstructed_on_parsed = cov_matrix_quantum[part_ind]
    #     exp_quant_matrix_error_parsed = cov_matrix_reconstructed_std[part_ind]
    #     quant, quant_err = search_eval( partition_list, reconstructed_on_parsed, exp_quant_matrix_error_parsed, 2 )
    #     kpart_val[part_ind, ii-1] = quant
    #     kpart_err[part_ind, ii-1] = quant_err
        
    #     ii += 1
    
    # print( "The k-1 partition criteria value is", np.max( kpart_val[part_ind] / kpart_err[part_ind] ) )
    ...
    #         bipart_val[part_ind, ii-1] = quant
    #         bipart_err[part_ind, ii-1] = quant_err
    ...
    # print( "The general bipartition criteria value is", np.max( bipart_val[part_ind] / bipart_err[part_ind]) )
...
# Plotting the reconstruction value
fig, ax = plt.subplots(1)
ax.plot( optimal_value, '-' )
ax.set_title( 'Reconstruction Value' )
ax.set_xlabel( 'Chunk index' )
ax.set_ylabel( 'Optimal value' )


# Plotting the PPT entanglement test values
ppt_mean = np.mean( min_eig_ppt, axis=0 )

fig, ax = plt.subplots(1)
ax.plot(ppt_mean, '.')
ax.set_title( 'PPT test' )
ax.set_xlabel( 'bipartition index')
...
zmax = np.max( np.mean( cov_matrix, axis=0 ) )
zmin = -zmax

fig1, ax1 = plt.subplots(1)
a = ax1.pcolormesh( x, x, np.flipud( np.mean( cov_matrix, axis=0 ) ), shading='nearest', cmap='RdBu_r', vmin=zmin, vmax=zmax )
...
x = np.arange( -1 + nmodes[0], nmodes[-1]+2, 1)
grid_arr = np.arange( -1 + nmodes[0], nmodes[-1]+2, 2)
zmax = np.max( np.mean( cov_matrix_reconstructed, axis=0 ) )
zmin = -zmax

fig1, ax1 = plt.subplots(1)
a = ax1.pcolormesh( x, x, np.flipud( np.mean( cov_matrix_reconstructed, axis=0 ) ), shading='nearest', cmap='RdBu_r', vmin=zmin, vmax=zmax )
...
x = np.arange( -1 + nmodes[0], nmodes[-1]+2, 1)
grid_arr = np.arange( -1 + nmodes[0], nmodes[-1]+2, 2)
zmax = np.max( np.mean( cov_matrix_quantum, axis=0 ) )
zmin = -zmax

fig1, ax1 = plt.subplots(1)
a = ax1.pcolormesh( x, x, np.flipud( np.mean( cov_matrix_quantum, axis=0 ) ), shading='nearest', cmap='RdBu_r', vmin=zmin, vmax=zmax )

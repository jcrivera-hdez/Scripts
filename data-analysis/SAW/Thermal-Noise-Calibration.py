...
def thermal_noise_func( temperature, gain_signal, gain_idler, ns, f_signal, df ):
    ...
    # Thermal Noise
    V2_signal = factor_signal * ( Gs / np.tanh( Planck*f_signal / (2*Boltzmann*temperature*1e-3) ) + (Gs - 1)*(2*ns + 1) )
...
temp_folder = os.path.join(r'D:\VivaceData\Vivace-Calibration\vivace_temp_noise_calibration\2020-10\vivace_temp_noise_calibration-20201023.hdf5')
...
# Temperature calibration data folder
folder = os.path.join( r'D:\VivaceData\Vivace-Calibration\vivace_temp_noise_calibration_2021-01\SAW_calibration_data_Jan_2021.hdf5' )
...
ax.plot( np.linspace(1,700,1000), thermal_noise_func(np.linspace(1,700,1000), gs[freq_ind], gi[freq_ind], ns[freq_ind], freq_used, df), '--', label = 'noise fit'  )
...
fig, ax = plt.subplots(1)
ax.errorbar( T, noise_power[freq_ind] / df, xerr = T_std, yerr = noise_power_std[freq_ind] / df, fmt = '.', label = 'Calibration' )
ax.plot( np.linspace(1,700,1000), thermal_noise_func(np.linspace(1,700,1000), gs[freq_ind], gi[freq_ind], ns[freq_ind], freq_comb[freq_ind], df) / df, '--' )
ax.errorbar( base_temp, base_noise[freq_ind+freq_disp] / df2, yerr = base_noise_std[freq_ind+freq_disp] / df2, fmt = '.', label = 'SAW pump off' )
ax.set_xlabel( 'Temperature (mK)' )
ax.set_ylabel( 'Spectral Noise Density' )
ax.legend()
...
for ns_ind, nslim in enumerate( ns_boundary ):
    ...
    for freq_ind in range(len(freq_comb)):
        ...
        # Fit quality parameters
        residuals = noise_power[freq_ind, :-x] - thermal_noise_func_constrained(T[:-x], gs_cons[ns_ind, freq_ind], ns_cons[ns_ind, freq_ind], freq_used, df)
...
fig, ax = plt.subplots(1)
ax.errorbar( T, noise_power[freq_ind], xerr = T_std, yerr = noise_power_std[freq_ind] , fmt = '.', label = 'measurement' )
ax.plot( np.linspace( 1,700,1000), thermal_noise_func_constrained( np.linspace(1,700,1000), gs_cons[0, freq_ind], ns_cons[0, freq_ind], freq_used, df ), '--', label = 'noise fit'  )
ax.set_xlabel( 'Temperature (mK)' )
ax.set_ylabel( 'Voltage noise ($\mu V^2$)' )
ax.set_title( 'Frequency = ' + str( "{:.4f}".format(freq_used / 1e9) ) + ' GHz (' + str(freq_ind) + ')' )
ax.legend()
